...
// Simulate a slight delay for Elasticsearch stream simulation
...
export async function SearchResults({ query }: { query: string }) {
  ...
  await dbConnect();
  await delay(1000); // Artificial delay to demonstrate Suspense streaming
    ...
    <div className="space-y-8">
        ...
        {candidates.length > 0 ? (
          <div className="grid gap-4">
            {candidates.map((c: any) => (
              <div key={c._id.toString()} className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 flex items-center justify-between hover:border-indigo-500 transition-all cursor-pointer group">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="font-bold text-slate-900 dark:text-white group-hover:text-indigo-600 transition-colors">{c.name}</p>
      ...
      <section className="space-y-4">
        ...
        {jobs.length > 0 ? (
          <div className="grid gap-4">
            {jobs.map((j: any) => (
              <div key={j._id.toString()} className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 flex items-center justify-between hover:border-indigo-500 transition-all cursor-pointer group">
                <div>
                  <p className="font-bold text-slate-900 dark:text-white group-hover:text-indigo-600 transition-colors">{j.title}</p>
