...
export default function CandidateTable({ initialData }: CandidateTableProps) {
  ...
  // Sync state with server props when they change (e.g. due to filtering)
  React.useEffect(() => {
  ...
  const columnDefs = useMemo<ColDef[]>(() => [
    ...
    { 
      ...
      cellRenderer: (params: ICellRendererParams) => {
        ...
        return (
          <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${styles[params.value] || 'bg-slate-50'}`}>
            {params.value}
          </span>
        );
      }
          ...
          <button 
            onClick={async () => {
              if (confirm('Are you sure you want to delete this candidate?')) {
                ...
                setRowData(prev => prev.filter(c => c._id !== params.data._id));
              }
            }}
